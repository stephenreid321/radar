$(function () {
  ...
  $.get(`${BASE_URI}/channels`, function (data) {
    $(data).each(function (i, channel) {
      const categoryBlock = $('.category-block').first().clone()
      ...
      $(channel.tags).each(function (i, tagName) {
        const tag = categoryBlock.find('.tags').first().clone()
        tag.text(tagName)
  ...
  const searchForm = $('<form style="height: 100%"><input name="channel" type="hidden"><input name="q" type="text" style="padding-left: 0.5em; width: 100%; height: 100%" /></form>')
  ...
  $.get(`${BASE_URI}/links?${$.param({ channel: urlParams.get('channel'), tags: urlParams.getAll('tags[]'), q: urlParams.get('q') })}`, function (data) {
    $(data).each(function (i, link) {
      ...
      resourceBlock.find('.resource-title').text('').html(truncate(link.data.title, 100, true) || truncate(link.data.description, 100, true) || truncate(link.data.url.replace(/^https?:\/\//, '').replace(/^www\./, ''), 44))
      ...
      if (link.data.description) { resourceBlock.find('.resource-description').html(`<p class="description-paragraph">${link.data.description}`).click(function () { window.open(link.data.url) }).css('cursor', 'pointer') }
      ...
      $(link.tagships).each(function (i, tagship) {
        const tag = resourceBlock.find('.tags').first().clone()
